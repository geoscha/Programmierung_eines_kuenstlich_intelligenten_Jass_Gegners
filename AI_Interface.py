# Graphische Nutzer Oberfläche für das Jass Programm mit künstlicher Intelligenz
...
trumpf_tabelle = [[0, 0, 0, 14, 10, 20, 3, 4, 11], [0, 0, 0, 0, 10, 2, 3, 4, 11]]
obe_abe_tabelle = [0, 0, 8, 0, 10, 2, 3, 4, 11]
une_ufe_tabelle = [11, 0, 8, 0, 10, 2, 3, 4, 0]
...
farben = [pygame.image.load("/Users/georgschaeli/Desktop/Programmierung_eines_kuenstlich_intelligenten_Jass_Gegners/Data/" + i + ".png") for i in
...
def get_punkte(x):  # gibt Punktewert einer Karte zurück
    ...
    if get_col(x) == trumpf:
        return trumpf_tabelle[0][get_typ(x)]
    elif modus == 0:
        return obe_abe_tabelle[get_typ(x)]
    elif modus == 1:
        return une_ufe_tabelle[get_typ(x)]
    else:
        return trumpf_tabelle[1][get_typ(x)]
...
def get_AI_pred(hand: object, used: object, spieler: object, ansage_ai, reps) -> object:
    ...
    for g in range(reps):
        ...
        model_no = modus
        if ansage_ai:
            x = [situation_to_x([hand_erg(hand[spieler], used), hand[spieler]], used, spieler, i, False) for i in mögl]
        else:
            x = [situation_to_x([hand_erg(hand[spieler], used), hand[spieler]], used, spieler, spielfeld[0], i) for i in mögl]
            model_no += 6
...
def ausspielen(hand, ansager, karte_1):  # Minimax Ansage/Reaktion
    ...
    if performance and not karte_1:  # Baumtiefenuntersuchung: Ansagen
        rechen_schritte = [2, 2, 2, 2, 3, 3, 4, 5, 4, 3, 2, 0]
    elif performance:  # Baumtiefenuntersuchung: Reaktion
        rechen_schritte = [2, 2, 2, 3, 4, 4, 5, 5, 4, 3, 2, 0]
    else:  # Baumtiefenuntersuchung: Modus Ansagen, Böcke finden
        rechen_schritte = [1, 1, 1, 1, 2, 2, 2, 3, 3, 3, 2, 0]
...
while spielaktiv:
    ...
    if status == -2:  # Begrüssung, Projektvorstellung
        ...
        for ii in range(6):
            ...
            write_col("Neues Spiel wird vorbereitet", 10, 10, 10, WHITE)
            write_col("Laden...", 345, 290, 10, WHITE)
            write_col(str(round(v, 2)) + "%", 305, 290, 10, WHITE)
            pygame.display.flip()
        ...
        pygame.display.flip()
        if not spielfeld:
            spielfeld.append(get_AI_pred(hand, spielverlauf, spieler, True, 25))  # KNN sagt an
            ...
            try:
                empfehlung = get_AI_pred(hand, spielverlauf, spieler, False, 15) # Frei regulierbar
            except:
                max_punkte = []
                ...
                else:   # Es kann nur verloren werden
